...
function computeAllLoansData(){
  ...
  return loans.map((loan,i)=>{
    ...
    const budget=effectiveBudget(loan.id||i);
    const{rows,sched}=genProj(budget,loan.balance,startKey,rate,ey,em,
      getManLumps(loan.id||i),loan.lumpMonths,getActuals(loan.id||i),
...
// Safe stubs — implemented in Tasks 9/10. Present so renderDashboard doesn't throw.
